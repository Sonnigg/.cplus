...
)
{
    ...
    // Safety guard against zero division if execution is instantaneous
    ...
    printf("  Source Lines : %10zu\n", source_lines);
    printf("  Source Bytes : %10zu bytes\n", source_bytes);
    printf("  Tokens       : %10zu\n", token_count);

    printf("\n[ Output Metrics ]\n");
    printf("  Gen. Lines   : %10zu\n", generated_lines);
    printf("  Gen. Bytes   : %10zu bytes\n", generated_bytes);
...
int main(int argc, char **argv)
{
    ...
    double preprocess_start;
    double lex_start;
    double transpile_start;
    ...
    // always one because the last line will not contain \n and will also have \0
    size_t source_lines = 1;
    size_t source_bytes = 1;
    // can be 0
    size_t token_count = 0;
    // always one because the last line will not contain \n and will also have \0
    size_t generated_lines = 1;
    size_t generated_bytes = 1;
    ...
    generated_bytes = t.output.len;
    for (size_t x = 0; x < generated_bytes; x++) {
        if (t.output.data[x] == '\n') generated_lines++;
    }
    ...
    if (emit_c) {
        printf("C+ -> C: %s\n", generated);
        if (stats) {
            double total = timer_now() - total_start;
            print_stats(
                source_lines, source_bytes, token_count,
                generated_lines, generated_bytes,
                preprocess_time, lex_time, transpile_time, 0, total
            );
        }
        goto success_label;
    }
    ...
    if (stats) {
        ...
        print_stats(
            source_lines, source_bytes, token_count,
            generated_lines, generated_bytes,
            preprocess_time, lex_time, transpile_time, backend_time, total
        );
    }
